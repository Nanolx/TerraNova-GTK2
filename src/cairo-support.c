 ...
 ***********************************************/
void
tn_hsb_from_color (const CairoColor *color, 
                        gdouble *hue, 
                        gdouble *saturation,
                        gdouble *brightness) 
{
...
void
tn_gdk_color_to_cairo (const GdkColor *c, CairoColor *cc)
{
	gdouble r, g, b;
...
void
tn_cairo_color_to_gtk (const CairoColor *cc, GdkColor *c)
{
	gdouble r, g, b;
...
void 
tn_gtk_style_to_cairo_color_cube (GtkStyle * style, CairoColorCube *cube)
{
	int i;
	...
	for (i = 0; i < 5; i++)
	{
		tn_gdk_color_to_cairo (&style->bg[i], &cube->bg[i]);
		tn_gdk_color_to_cairo (&style->fg[i], &cube->fg[i]);

		tn_gdk_color_to_cairo (&style->dark[i], &cube->dark[i]);
		tn_gdk_color_to_cairo (&style->light[i], &cube->light[i]);
		tn_gdk_color_to_cairo (&style->mid[i], &cube->mid[i]);

		tn_gdk_color_to_cairo (&style->base[i], &cube->base[i]);
		tn_gdk_color_to_cairo (&style->text[i], &cube->text[i]);
		tn_gdk_color_to_cairo (&style->text_aa[i], &cube->text_aa[i]);
	}
...
void
tn_shade_color(const CairoColor *base, gdouble shade_ratio, CairoColor *composite)
{
	gdouble hue = 0;
	...
	tn_hsb_from_color (base, &hue, &saturation, &brightness);
	...
	tn_color_from_hsb (hue, saturation, brightness, composite);
	composite->a = base->a;	
}

void
tn_saturate_color (const CairoColor *base, gdouble saturate_level, CairoColor *composite)
{
	gdouble hue = 0;
	gdouble saturation = 0;
	gdouble brightness = 0;
...
void
tn_mix_color (const CairoColor *color1, const CairoColor *color2,
...
cairo_t * 
tn_gdk_drawable_to_cairo (GdkDrawable  *window, GdkRectangle *area)
{
	cairo_t *cr;
...
void 
tn_cairo_set_color (cairo_t *cr, const CairoColor *color)
{
...
void
tn_cairo_set_gdk_color_with_alpha (cairo_t *cr, const GdkColor *color, gdouble alpha)
{
	g_return_if_fail (cr && color);
...
void 
tn_cairo_pattern_add_color_stop_color (cairo_pattern_t *pattern, 
                                       gfloat offset, 
...
void
tn_cairo_pattern_add_color_stop_shade (cairo_pattern_t *pattern, 
                                       gdouble offset, 
                                       const CairoColor *color, 
                                       gdouble shade)
{
	...
	if (shade != 1)
	{
		tn_shade_color(color, shade, &shaded);
	}

	tn_cairo_pattern_add_color_stop_color(pattern, offset, &shaded);
}
 ...
 * tn_cairo_rounded_corner assumes clockwise drawing.
 */
void
tn_cairo_rounded_corner (cairo_t      *cr,
                         double        x,
                         double        y,
                         double        radius,
                         CairoCorners  corner)
...
void
tn_cairo_rounded_rectangle (cairo_t *cr,
                                 ...
                                 double radius, CairoCorners corners)
{
	...
	/*if ((corners == CR_CORNER_ALL) && (radius > w / 2.0 || radius > h / 2.0))
		g_warning ("Radius is too large for width/height in tn_rounded_rectangle.\n");
 ...
 */
void
tn_cairo_stroke_rectangle (cairo_t *cr, double x, double y, double w, double h)
{
	cairo_rectangle (cr, x, y, w, h);
	cairo_stroke (cr);
}
...
void
tn_cairo_inner_rectangle (cairo_t *cr,
                          double x, double y,
...
void
tn_cairo_inner_rounded_rectangle (cairo_t *cr,
                                  double x, double y,
                                  double width, double height,
                                  double radius, CairoCorners corners)
{
 ...
 ***********************************************/
void
tn_cairo_simple_border (cairo_t *cr,
...
void tn_cairo_polygon (cairo_t *cr,
                       const CairoColor *color,
                       GdkPoint *points,
                       gint npoints)
{
...
void
tn_cairo_exchantn_axis (cairo_t  *cr,
                        gint     *x,
                        gint     *y,
                        gint     *width,
                        gint     *height)
...
/***********************************************
 * tn_cairo_pattern_fill -
 *  
 *   Fill an area with some pattern
 *   Scaling or tiling if needed
 ***********************************************/
void 
tn_cairo_pattern_fill(cairo_t *canvas,
...
/***********************************************
 * tn_cairo_color_pattern -
 *  
 *   Create A Solid Color Pattern
 ***********************************************/
CairoPattern*
tn_cairo_color_pattern(CairoColor *base)
{
...
/***********************************************
 * tn_cairo_pixbuf_pattern -
 *  
 *   Create A Tiled Pixbuf Pattern
 ***********************************************/
CairoPattern*
tn_cairo_pixbuf_pattern(GdkPixbuf *pixbuf)
{
	CairoPattern * result = g_new0(CairoPattern, 1);
...
/***********************************************
 * tn_cairo_pixmap_pattern -
 *  
 *   Create A Tiled Pixmap Pattern
 ***********************************************/
CairoPattern*
tn_cairo_pixmap_pattern(GdkPixmap *pixmap)
{	
	...
	result = tn_cairo_pixbuf_pattern(pixbuf);
...
/***********************************************
 * tn_cairo_linear_shade_gradient_pattern - 
 *  
 ...
 ***********************************************/
CairoPattern *
tn_cairo_linear_shade_gradient_pattern(CairoColor *base, 
                                       gdouble shade1, 
                                       gdouble shade2, 
                                       gboolean vertical)
{
	...
	tn_cairo_pattern_add_color_stop_shade(result->handle, 0, base, shade1);
	tn_cairo_pattern_add_color_stop_shade(result->handle, 1, base, shade2);

	return result;
}

void
tn_cairo_pattern_destroy(CairoPattern *pattern)
{
	if (pattern)
	{
		if (pattern->handle)
